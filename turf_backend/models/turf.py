...
class Horse(SQLModel, table=True):
    ...
    race_id: UUID = Field(default=uuid4(), foreign_key="races.race_id", index=True)
    numero: str | None = Field(default=None, index=True)
    nombre: str | None = Field(default=None, index=True)
    peso: int | None = Field(default=None)
    jockey: str | None = Field(default=None, index=True)
