        ...
        for item in sublist:
            if isinstance(item, list) and item and item[0] and 'Carrera' in item[0]:
                if current_race:
                    ...
                    current_race = [[elem for elem in entry if elem is not None and elem != ''] for entry in current_race if any(entry)]
                    races.append(Race(name, time, current_race))
                name, time = item
                current_race = [item]
            elif any(isinstance(i, str) and 'Caballeriza' in i for i in item):
                current_race.append(item)
            elif any(isinstance(i, str) and i for i in item):
                current_race.append(item)
